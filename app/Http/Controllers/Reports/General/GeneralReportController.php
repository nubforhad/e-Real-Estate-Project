...
class GeneralReportController extends Controller
{
    ...
    public function ledger_name_report(Request $request)
    {
        ...
        if (!empty($request->from)) {
            ...
            $dateTo = new \DateTime($request->to);
            $To = $dateTo->format('Y-m-d'); // 31-07-2012 '2008-11-11'
            $items = IncomeExpenseHead::with('IncomeExpenseType', 'IncomeExpenseGroup')->whereBetween('created_at', [$From . " 00:00:00", $To . " 23:59:59"])
                ->orderBy('created_at', 'asc')
                ->get();
            $from = date(config('settings.date_format'), strtotime($request->start_from));
            $to = date(config('settings.date_format'), strtotime($request->to));
        } else {
            $items = IncomeExpenseHead::with('IncomeExpenseType', 'IncomeExpenseGroup')->get();
            $from = 'UpTo ' . $date;
            $to = 'UpTo ' . $date;
        }
        if (count($items) < 1) {
    ...
    public function voucher_report(Request $request)
    {
        ...
        if (empty($request->from) and empty($request->voucher_type)) {
            $items = Transaction::with('Branch', 'IncomeExpenseHead', 'BankCash')->get();
            $from = 'UpTo ' . $date;
            $to = 'UpTo ' . $date;
            ...
            $dateTo = new \DateTime($request->to);
            $To = $dateTo->format('Y-m-d'); // 31-07-2012 '2008-11-11'
            $items = Transaction::with('Branch', 'IncomeExpenseHead', 'BankCash')->where('voucher_type', $request->voucher_type)
            ...
            $dateTo = new \DateTime($request->to);
            $To = $dateTo->format('Y-m-d'); // 31-07-2012 '2008-11-11'
            $items = Transaction::with('Branch', 'IncomeExpenseHead', 'BankCash')->whereBetween('created_at', [$From . " 00:00:00", $To . " 23:59:59"])
                ->orderBy('created_at', 'asc')
                ->get();
            $from = date(config('settings.date_format'), strtotime($request->start_from));
            $to = date(config('settings.date_format'), strtotime($request->to));
