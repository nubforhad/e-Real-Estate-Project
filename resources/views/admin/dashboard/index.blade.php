                ...
                <div class="col-lg-3 col-md-3 col-sm-4 col-xs-12">
                    <div class="info-box bg-blue-grey hover-expand-effect">
                        ...
                        <div class="content">
                            ...
                            <div class="number count-to" data-from="0" data-to="{{ $total_bank_cashes }}"
                                data-speed="1000" data-fresh-interval="20"></div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card">
                        ...
                        <div class="body">
                            <div class="button-demo">
                                <a href="{{ route('branch') }}" type="button"
                                    class="btn bg-pink waves-effect">{{ __('root.dashboard.branch') }}</a>
                                <a href="{{ route('product') }}" type="button"
                                    class="btn bg-green waves-effect">{{ __('root.dashboard.product') }}</a>
                                <a href="{{ route('sell') }}" type="button"
                                    class="btn bg-amber waves-effect">{{ __('root.dashboard.sell') }}</a>
                                <a href="{{ route('purchase_requisition') }}" type="button"
                                    class="btn bg-black waves-effect">{{ __('root.dashboard.purchase_requisition') }}</a>
                                <a href="{{ route('purchase_order') }}" type="button"
                                    class="btn bg-cyan waves-effect">{{ __('root.dashboard.purchase_order') }}</a>
                                <a href="{{ route('vendor') }}" type="button"
                                    class="btn bg-green waves-effect">{{ __('root.dashboard.vendor') }}</a>
                                <a href="{{ route('employee') }}" type="button"
                                    class="btn bg-amber waves-effect">{{ __('root.dashboard.employee') }}</a>
                                <a href="{{ route('customer') }}" type="button"
                                    class="btn bg-black waves-effect">{{ __('root.dashboard.customer') }}</a>
                            ...
                            <div class="row">
                                ...
                                <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
                                    <div class="form-group form-float">
                                        <div class="form-line">
                                            <select data-live-search="true" class="form-control  show-tick"
                                                name="branch_id" id="voucher_branch">
                                                <option value="0">Select branch</option>
                                                @foreach ($branches as $branche)
                                                    <option value="{{ $branche->id }}">{{ $branche->name }}</option>
                                                @endforeach
...
@push('include-js')
    ...
    <script src="{{ Helper::assetV('asset/plugins/jquery-countto/jquery.countTo.js') }}"></script>
    <!-- ChartJs -->
    <script src="{{ Helper::assetV('asset/plugins/chartjs/Chart.min.js') }}"></script>
    <script>
        ...
        $(function() {
            ...
            function getTotalVoucher(year, branch_id = 0) {
                return new Promise((resolve, reject) => {
                    let datasets;
                    Helper.ajaxRequest('GET', `total-voucher?year=${year}&branch_id=${branch_id}`).then((
                        res) => {
                        let data = res.data;
                        datasets = [{
            ...
            function getProfitOrLoss(year, branch_id = 0) {
                return new Promise((resolve, reject) => {
                    let datasets;
                    Helper.ajaxRequest('GET', `profit-loss?year=${year}&branch_id=${branch_id}`).then((
                        res) => {
            ...
            document.getElementById("voucher_year").addEventListener('change', (e) => {
                var branch_id = document.getElementById('voucher_branch').value;
                getTotalVoucher(e.target.value, branch_id).then((datasets) => {
                    barChart.destroy();
                    barChart = new Chart(bar_chart, Helper.configChart('bar', labels, datasets));
                }).catch((error) => {
                    console.log(error);
            ...
            document.getElementById("voucher_branch").addEventListener('change', (e) => {
                var branch_id = (e.target.value > 0) ? e.target.value : 0;
                var voucher_year = document.getElementById('voucher_year').value;
                getTotalVoucher(voucher_year, branch_id).then((datasets) => {
                    barChart.destroy();
                    barChart = new Chart(bar_chart, Helper.configChart('bar', labels, datasets));
                }).catch((error) => {
                    console.log(error);
            ...
            document.getElementById("profit_or_loss_year").addEventListener('change', (e) => {
                ...
                getProfitOrLoss(e.target.value, branch_id).then((datasets) => {
            ...
            document.getElementById("branch_id").addEventListener('change', (e) => {
                var year = document.getElementById('profit_or_loss_year').value;
                getProfitOrLoss(year, e.target.value).then((datasets) => {
                    profitOrLossGraph.destroy();
                    profitOrLossGraph = new Chart(profit_or_loss_graph, Helper.configChart('line',
                        labels, datasets));
                }).catch((error) => {
